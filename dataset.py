...
def _looks_like_millis(ts: int) -> bool:
    """Heuristic for distinguishing epoch milliseconds from seconds."""
    return abs(int(ts)) >= 10 ** 11


def _timestamp_to_datetime(ts: int, tz_offset_hours: int) -> datetime:
    """Convert an integer epoch timestamp to a timezone-aware datetime."""
    tz = timezone(timedelta(hours=tz_offset_hours))
    scale = 1000.0 if _looks_like_millis(ts) else 1.0
    return datetime.fromtimestamp(float(int(ts)) / scale, tz=tz)


def _day_range_to_epoch(
    year: int,
    month: int,
    day: int,
    tz_offset_hours: int,
    ...
    factor = 1000 if _looks_like_millis(reference_ts) else 1
...
def _extract_time_features(
    ...
    """Extract day-of-week, hour-of-day, and cyclic hour encoding from timestamps.
    ...
    for i, ts in enumerate(timestamps):
        dt = _timestamp_to_datetime(int(ts), tz_offset_hours)
        day_of_week[i] = dt.weekday() + 1          # Monday=1 ... Sunday=7
        hour = dt.hour
        hour_id[i] = hour + 1                      # 1-24
        angle = 2.0 * math.pi * hour / 24.0
        hour_sin[i] = math.sin(angle)
        hour_cos[i] = math.cos(angle)
    return day_of_week, hour_id, hour_sin, hour_cos


def _parse_valid_date_range(
    valid_date: str,
    reference_ts: int,
    tz_offset_hours: int,
) -> Tuple[int, int, str]:
    """Parse ``MM-DD`` / ``YYYY-MM-DD`` into [start, end) epoch bounds."""
    normalized = valid_date.strip().replace('/', '-').replace('.', '-')
    parts = [p for p in normalized.split('-') if p]
    if len(parts) == 2:
        ref_dt = _timestamp_to_datetime(reference_ts, tz_offset_hours)
        year = ref_dt.year
    ...
    start_ts, end_ts = _day_range_to_epoch(
        year=year,
        month=month,
        day=day,
...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        Args:
            ...
            timestamp_range: optional sample-level timestamp window
    ...
    def __iter__(self) -> Iterator[Dict[str, Any]]:
        ...
        for file_path, rg_idx, _ in rg_list:
            ...
            for batch in pf.iter_batches(batch_size=self.batch_size, row_groups=[rg_idx]):
                ...
                batch_dict = self._filter_converted_batch_by_timestamp(batch_dict)
    ...
    def _filter_converted_batch_by_timestamp(
        self,
        batch_dict: Dict[str, Any],
    ) -> Optional[Dict[str, Any]]:
        """Filter a converted batch by ``self.timestamp_range`` when configured."""
        if self.timestamp_range is None:
            return batch_dict

        start_ts, end_ts = self.timestamp_range
        timestamps = batch_dict['timestamp']
        keep_mask = torch.ones_like(timestamps, dtype=torch.bool)
        if start_ts is not None:
            keep_mask &= timestamps >= int(start_ts)
        if end_ts is not None:
            keep_mask &= timestamps < int(end_ts)
        ...
        for key, value in batch_dict.items():
            if isinstance(value, torch.Tensor):
                ...
                filtered[key] = [value[int(i)] for i in keep_idx.tolist()]
            else:
                filtered[key] = value
        return filtered
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        sample_day_ids, sample_hour_ids, sample_hour_sin, sample_hour_cos = _extract_time_features(timestamps, 8)
...
def get_pcvr_data(
    ...
    **kwargs: Any,
    ...
    first_pf = pq.ParquetFile(pq_files[0])
    schema_names = first_pf.schema_arrow.names
    if 'timestamp' not in schema_names:
        raise KeyError("timestamp column is required for PCVR parquet loading")
    ts_col_idx = schema_names.index('timestamp')
    ...
    train_row_groups: List[Tuple[str, int, int]]
    valid_row_groups: List[Tuple[str, int, int]]
    ...
    if valid_split_mode == 'timestamp':
        if valid_start_ts is None and valid_date:
            reference_ts = next(
                (mx for _, _, _, _, mx in reversed(rg_info) if mx is not None),
                None,
            )
            ...
            valid_start_ts, valid_end_ts, canonical_day = _parse_valid_date_range(
                str(valid_date),
                reference_ts=reference_ts,
        ...
        train_timestamp_range = (None, int(valid_start_ts))
        valid_timestamp_range = (int(valid_start_ts), int(valid_end_ts))

        train_row_groups = [
            (f, i, n)
            for f, i, n, mn, mx in rg_info
            if _timestamp_range_overlaps(mn, mx, train_timestamp_range[0], train_timestamp_range[1])
        ]
        valid_row_groups = [
            (f, i, n)
            for f, i, n, mn, mx in rg_info
            if _timestamp_range_overlaps(mn, mx, valid_timestamp_range[0], valid_timestamp_range[1])
        ]

        if train_ratio < 1.0:
            kept = max(1, int(len(train_row_groups) * train_ratio))
            train_row_groups = train_row_groups[:kept]
            logging.info(
        ...
        train_row_groups = [(f, i, n) for f, i, n, _, _ in rg_info[:n_train_rgs]]
        valid_row_groups = [(f, i, n) for f, i, n, _, _ in rg_info[n_train_rgs:]]
        train_rows = sum(r[2] for r in train_row_groups)
        valid_rows = sum(r[2] for r in valid_row_groups)
        logging.info(

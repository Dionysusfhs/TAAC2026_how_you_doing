...
class PCVRHyFormerRankingTrainer:
    ...
    def _build_step_dir_name(
        ...
        ``global_step2500.layer=2.head=4.hidden=64[.epoch8][.best_model]``.
        """
    ...
    def _save_fixed_epoch_checkpoint(
        ...
        """Save one extra checkpoint at a configured epoch, independent of best-model logic."""
        if self.save_epoch_ckpt <= 0 or epoch != self.save_epoch_ckpt:
            return
        ...
        if os.path.exists(os.path.join(ckpt_dir, "model.pt")):
            logging.info(f"Fixed epoch checkpoint already exists, skip: {ckpt_dir}/model.pt")
            return
        if use_ema:
            ...
            self._save_step_checkpoint(total_step, extra_tag=extra_tag)
        logging.info(f"Saved fixed epoch checkpoint for epoch {epoch}: {ckpt_dir}/model.pt")

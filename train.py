...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--save_epoch_ckpt', type=int, default=8,
                        help='Also save one extra checkpoint after validation at this epoch '
                             '(0 = disable this fixed-epoch checkpoint)')
    ...
    parser.add_argument('--ema_decay', type=float, default=0.9995,
                        help='Dense-parameter EMA decay. Set <=0 to disable EMA.')
    parser.add_argument('--ema_start_step', type=int, default=100,
                        help='Start updating dense-parameter EMA after this global step.')
    parser.add_argument('--ema_update_every', type=int, default=1,
                        help='Update dense-parameter EMA every N optimizer steps.')
    parser.add_argument('--eval_raw_and_ema', action='store_true', default=True,
                        help='Evaluate both raw and EMA dense weights, then save the higher-AUC one.')
    parser.add_argument('--no_eval_raw_and_ema',
                        dest='eval_raw_and_ema', action='store_false',
                        help='Evaluate only raw weights.')
    ...
    parser.add_argument('--use_user_dense_residual', action='store_true', default=True,
                        help='Enable zero-initialized residual encoder over user dense features.')
    parser.add_argument('--no_user_dense_residual',
                        dest='use_user_dense_residual', action='store_false',
                        help='Disable the user dense residual branch.')
    parser.add_argument('--use_din_residual', action='store_true', default=True,
                        help='Enable zero-initialized DIN-style target-aware sequence residual.')
    parser.add_argument('--no_din_residual',
                        dest='use_din_residual', action='store_false',
                        help='Disable the DIN-style sequence residual branch.')
    parser.add_argument('--use_dcn_residual', action='store_true', default=True,
                        help='Enable zero-initialized DCNv2-style non-sequence residual.')
    parser.add_argument('--no_dcn_residual',
                        dest='use_dcn_residual', action='store_false',
                        help='Disable the DCNv2-style non-sequence residual branch.')
    parser.add_argument('--use_aux_logit_residual', action='store_true', default=True,
                        help='Enable zero-initialized auxiliary residual logit head.')
    parser.add_argument('--no_aux_logit_residual',
                        dest='use_aux_logit_residual', action='store_false',
                        help='Disable the auxiliary residual logit head.')
    parser.add_argument('--din_top_k', type=int, default=80,
                        help='Latest valid sequence tokens used by the DIN residual branch.')
    parser.add_argument('--dcn_low_rank', type=int, default=128,
                        help='Low-rank width used by the DCNv2 residual branch.')

    _default_ns_groups = os.path.join(SCRIPT_DIR, 'ns_groups.json')
    ...
    args.tf_events_dir = os.environ.get(
        ...
        os.path.join(args.log_dir, 'events') if args.log_dir else None,
    )
...
def main() -> None:
    ...
    model_args = {
        ...
        "use_user_dense_residual": args.use_user_dense_residual,
        ...
        "dcn_low_rank": args.dcn_low_rank,
    ...
    trainer = PCVRHyFormerRankingTrainer(
        ...
        eval_raw_and_ema=args.eval_raw_and_ema,
    )

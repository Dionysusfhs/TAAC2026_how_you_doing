...
# ckpt directory.
#
# These follow this variant's ``run.sh`` mainline. In normal evaluation
# ``train_config.json`` from the checkpoint remains the source of truth; these
# values are only used when that sidecar is missing.
#
...
_FALLBACK_MODEL_CFG = {
    ...
    'use_target_history_matching': True,
    'ns_tokenizer_type': 'rankmixer',
    'user_ns_tokens': 5,
    'item_ns_tokens': 2,
    # DenseGroup MaxAUC SOTA residual branches (all enabled by default).
    'use_user_dense_groups': True,
    'use_user_dense_residual': True,
    'use_din_residual': True,
    'use_dcn_residual': True,
    'use_aux_logit_residual': True,
    'din_top_k': 80,
    'dcn_low_rank': 128,
}
...
def build_model(
    ...
    # User dense feature specs — required by densegroup_maxauc for the
    # grouped dense projection and residual branch.
...
def _batch_to_model_input(
    ...
    return ModelInput(
        ...
        sample_day_id=device_batch.get('sample_day_id', torch.zeros(B, dtype=torch.long, device=device)),
        sample_hour_id=device_batch.get('sample_hour_id', torch.zeros(B, dtype=torch.long, device=device)),
        sample_hour_sin=device_batch.get('sample_hour_sin', torch.zeros(B, dtype=torch.float32, device=device)),
        sample_hour_cos=device_batch.get('sample_hour_cos', torch.zeros(B, dtype=torch.float32, device=device)),
    )
